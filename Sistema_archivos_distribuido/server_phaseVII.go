...
// cleaner() ahora usará el protocolo de gossip para validar los TTLs
func cleaner() {
		...
		for key, entry := range sharedFiles {
			if entry.TTL > 0 {
				...
				if entry.TTL <= 0 {
					...
					foundNewOwner := false
					// Pide a un peer aleatorio el estado del archivo
					...
					if len(peersToCheck) > 0 {
						...
						if err == nil {
							// Se encontró un nuevo dueño. Actualiza el registro.
							sharedFilesMutex.RUnlock()
							sharedFilesMutex.Lock()
...
func main() {
	...
	// La dirección del propio servidor para que no se añada a su propia lista de peers.
	...
	// Inicia la rutina de chismorreo periódico.
...
func handleClient(conn net.Conn) {
	...
	// Agrega el cliente como un peer potencial para el chismorreo
