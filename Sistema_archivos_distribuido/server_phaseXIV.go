...
// Las estructuras DirectoryEntry, NetworkMessage, FileUpdate,
// logEntry, y logEvent se han movido a structs.go y son accedidas desde ahí.
...
// ttlChecker verifica los registros y realiza el consenso
func ttlChecker() {
	...
	for range ticker.C {
		...
		var updates map[string]DirectoryEntry
		...
		for key, entry := range sharedFiles {
			...
			if entry.TTL <= 0 {
				...
				// Desbloqueamos el mutex antes de hacer la llamada de red.
				...
				// Volvemos a bloquear para actualizar el mapa.
				sharedFilesMutex.Lock()
...
func handleClient(conn net.Conn) {
	...
	for {
		...
		case "REQUEST_FILE":
			...
			entry, found := sharedFiles[fileName]
			sharedFilesMutex.RUnlock()
			if found && entry.OwnerIP == selfAddr {
