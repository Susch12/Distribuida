...
var (
	...
	knownPeers       = []string{"localhost:8081"} // Asumimos que hay otro servidor en el puerto 8081
	gossipProtocol   *GossipProtocol
)

// cleaner() es ahora más inteligente.
func cleaner() {
	...
	for range ticker.C {
		...
		// Using a local slice to collect keys for deletion.
		var keysToDelete []string

		// Using a local map to collect updates.
		updates := make(map[string]DirectoryEntry)
		...
		for key, entry := range sharedFiles {
			if entry.TTL > 0 {
				...
				if entry.TTL <= 0 {
					...
					// Release the lock before a potentially blocking network call
					sharedFilesMutex.Unlock()

					foundNewOwner := false
					for _, peerAddr := range knownPeers {
						newEntry, err := gossipProtocol.RequestStatus(key, peerAddr)
						if err == nil {
							// Found a new authoritative owner.
		...
		// Apply collected changes to the shared map
		...
		// Unlocking at the very end of the loop
		sharedFilesMutex.Unlock()
	}
}

// initServerData es una nueva función para separar la lógica de inicialización
func initServerData() {
	sharedFiles["test_file.txt"] = DirectoryEntry{
		...
		TTL: 60,
		OwnerIP: "127.0.0.1:8080", // Este servidor es el dueño
	}
	sharedFiles["perpetual_file.doc"] = DirectoryEntry{
		...
		ModificationDate: time.Now(),
		TTL: 0,
		OwnerIP: "127.0.0.1:8080",
	}
	logEvent("SERVER", "DIRECTORY_INIT", "Directorio inicializado con archivos de prueba.")
}

func main() {
	// Cargar el certificado y la clave del servidor firmados por la CA
	...
	// Inicializar el protocolo de gossip
	gossipProtocol, err = NewGossipProtocol(knownPeers, dtlsConfig)
	if err != nil {
		panic(err)
	}
...
func handleClient(conn net.Conn) {
	...
	// Establece un tiempo de espera de lectura para evitar el bloqueo indefinido.
	...
	case "FILE_COPY_UPDATE":
		...
		if found && updatedEntry.ModificationDate.After(originalEntry.ModificationDate) {
			sharedFiles[updatedEntry.FileName] = updatedEntry
			logEvent("SERVER", "UPDATE_SUCCESS", fmt.Sprintf("Archivo '%s' actualizado con éxito. Nuevo dueño: %s", updatedEntry.FileName, updatedEntry.OwnerIP))
		} else {
			logEvent("SERVER", "UPDATE_REJECTED", fmt.Sprintf("Rechazada actualización de '%s'. La versión local es más reciente.", updatedEntry.FileName))
		}
		sharedFilesMutex.Unlock()
