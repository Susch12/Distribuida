...
type GossipProtocol struct {
    ...
    selfAddr        string
    heartbeatTimeout time.Duration // Tiempo máximo sin un heartbeat antes de considerar a un peer como caído
}
...
func NewGossipProtocol(peers []string, dtlsConfig *dtls.Config, selfAddr string) (*GossipProtocol, error) {
    gp := &GossipProtocol{
        ...
        selfAddr:         selfAddr,
        heartbeatTimeout: 60 * time.Second, // 60 segundos por defecto
    }
...
func (gp *GossipProtocol) AddPeer(peerAddr string) {
    gp.mu.Lock()
    defer gp.mu.Unlock()
    // Solo agrega si no es la dirección de este servidor
    if peerAddr != gp.selfAddr {
        if _, exists := gp.Peers[peerAddr]; !exists {
            ...
            // Actualiza LastSeen si el peer ya existe
...
func (gp *GossipProtocol) SendHeartbeat() {
    peersToSend := gp.GetRandomPeers(3) // Envía heartbeats a 3 peers al azar
    for _, peerAddr := range peersToSend {
        go func(addr string) {
            ...
            if err != nil {
                logEvent("HEARTBEAT", "ERROR", fmt.Sprintf("Falla al resolver dirección de peer %s: %v", addr, err))
                return
            }
...
// startGossip simplemente llama a la rutina de chismorreo en un goroutine
func StartGossip(gp *GossipProtocol) {
    go gp.StartGossipRoutine()
}

// Funciones RequestStatus y SendUpdate se mantienen igual
