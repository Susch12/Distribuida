			...
			if entry.TTL > 0 {
				...
				if entry.TTL <= 0 {
					...
					if len(peersToCheck) > 0 {
						...
						if err == nil {
							// Si el peer responde positivamente, actualiza el registro local.
							sharedFilesMutex.RUnlock()
							sharedFilesMutex.Lock()
					...
					if !foundNewOwner {
						// Si no se encuentra un nuevo dueño, se añade a la lista de eliminación.
...
func handleClient(conn net.Conn) {
	...
	gossipProtocol.AddPeer(conn.RemoteAddr().String())
