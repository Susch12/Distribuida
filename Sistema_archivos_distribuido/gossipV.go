...
type GossipProtocol struct {
	...
	selfAddr        string
	knownListenAddrs []string // La nueva lista de direcciones de escucha
...
func (gp *GossipProtocol) AddPeer(peerAddr string) {
	// Primero, verifica si la dirección del peer está en la lista de puertos de escucha conocidos.
	isKnownPeer := false
	...
	if !isKnownPeer {
		// Ignora el peer si su dirección no es un puerto de escucha conocido.
		// Esto evita que se añadan puertos efímeros a la lista.
		return
	}
